...
- `<base>` は拡張子を除いたファイル名（例: `sample_001`）。結合結果は `<base>_all.txt`（UTF-8 / BOM なし）。
  `transcribe-ffm` だけは結合ファイル名の日付を `yyyy-MM-dd` に展開します（後述）。
...
```powershell
# transcribe と同じ使い方。出力は入力と同階層の transcribe-ffm_<base>\
.\transcribe-ffm.bat "C:\Users\<user>\Music\260910_1539.MP3"

# フォルダ一括（transcribe-all の FFM 版）
.\transcribe-ffm-all.bat -d "C:\Users\<user>\Music"
```

**結合ファイル名は日付を展開します。** IC レコーダーのファイル名は先頭 6 桁が `YYMMDD` なので、
そこを `yyyy-MM-dd` に直した名前で結合ファイルを書き出します。

```
transcribe-ffm_260910_1539\
├── part_000.wav / part_000.txt      ← part は元のまま
├── part_001.wav / part_001.txt
└── 2026-09-10_1539_all.txt          ← 結合ファイルだけ日付形式
```

| 入力 | 結合ファイル |
| --- | --- |
| `260910_1539.MP3` | `2026-09-10_1539_all.txt` |
| `260619_003.MP3` | `2026-06-19_003_all.txt` |
| `260910.MP3` | `2026-09-10_all.txt` |
| `meeting.MP3` | `meeting_all.txt`（日付として読めないものはそのまま） |

先頭 6 桁が数字でない、区切りが `_` `-` 以外、月日が実在しない（`261340`、`260230`）場合は変換しません。
エラーにはせず元の名前を使います。

**フォルダ名は変えていません**（`transcribe-ffm_260910_1539` のまま）。フォルダ名を変えると既存の
出力フォルダが認識されず、「再実行するとスキップされる」という冪等性が壊れるためです。
`transcribe`（外部エンジン版）の出力名も従来どおり `<base>_all.txt` です。
...
| （そのうち短い行は数えない） | 8 文字以下は無視 | `transcribe.ffm.repetition-min-line-length` |
...
**短い行の連続は数えません。** 「はい。」のような相槌は自然に 5 行続くことがあり、実際に誤検知しました。
崩壊時に繰り返される行は実測 4 例すべてが 15 文字以上（幻聴も実発話の復唱も文の形をしている）なので、
8 文字以下の行は無視しています。回数を 6 に上げるのではなく長さで切っているのは、
**長い文が 5 行続くのは崩壊、相槌が 6 行続くのは自然**という区別をそのまま条件にできるためです。
...
1. 既定（VAD off）で流す
2. 警告が出た part の `.txt` を消して **`--vad` を付けて流し直す**
3. **それでも警告が出るなら、VAD off の結果に戻す**

**連続同一行が 1 ブロックにまとまっているなら、手で詰めるのが最善のこともあります。**
2026-09-10 の例では、VAD off は 20 行連続を含んで 3,434 文字、VAD on は繰り返しは消えたものの
1,692 文字（−51%）でした。VAD off から 20 行を 1 行に詰めると 2,864 文字が残り、これが最良でした。
`--vad` は繰り返しを消す代わりに実内容も削ることがあるので、**消えた文字数を必ず見比べてください。**

**同じ録音でも part ごとに勝者が変わります。** 2026-09-15 の 4 part 中 2 part が警告を出し、
片方は「詰める」が 1,712 文字（VAD on は 1,555）、もう片方は「VAD on」が 3,213 文字
（詰めると 2,777）で勝ちました。**part 単位で見比べてください。**

```powershell
$D = "C:\Users\<user>\Music\transcribe-ffm_<base>"

# 2. 消す前に退避してから、--vad で流し直す
#    退避名は part_ で始めないこと（part_NNN.txt 以外は結合対象にならないが、紛らわしいため）
Copy-Item "$D\part_000.txt" "$D\vadoff_000.txt"
Remove-Item "$D\part_000.txt"
.\transcribe-ffm.bat "C:\Users\<user>\Music\<base>.MP3" --vad

# 3. 悪化したら戻す（vadoff_000.txt を part_000.txt に戻し、もう一度実行して結合し直す）
```

> **退避ファイルの名前に注意。** 結合の対象は `part_<数字>.txt` だけです（`part_000_vadoff.txt` のような
> 名前は無視されます）。以前は「`part_` で始まり `.txt` で終わる」で拾っていたため、退避したコピーまで
> 結合されて同じ内容が二重に入っていました。今は弾かれますが、`vadoff_000.txt` のように
> `part_` で始まらない名前にしておく方が分かりやすいです。

**ステップ 3 が必要な理由。** `--vad` は崩壊を直すこともあれば、自分で崩壊を作ることもあります。
実測 6 パートの A/B です。

| part | 平均音量 | VAD off | VAD on | 判定 |
| --- | --- | --- | --- | --- |
| A | −26.0 dB | 崩壊（55 行連続 / 1.01 c/s） | 正常（3.83 c/s） | **VAD が直した** |
| B | −24.9 dB | 崩壊（14 行連続 / 0.72 c/s） | 正常（3.91 c/s） | **VAD が直した** |
| C | −23.3 dB | 崩壊（60 行連続 / 5.87 c/s） | 正常（6.00 c/s） | **VAD が直した** |
| D | −23.9 dB | 正常（5.12 c/s） | 正常（5.10 c/s） | −0.4% |
| E | −28.5 dB | 正常（4.58 c/s） | 正常（4.29 c/s） | −6.5% |
| F | −29.9 dB | 正常（4.02 c/s） | **崩壊（0.67 c/s）** | **VAD が壊した（−83%）** |

`c/s` は音声 1 秒あたりの文字数。**どちらに転ぶかを事前に予測する方法は見つかっていません。**
無音率でも音量でも説明しきれず、`vad-threshold` を 0.5 → 0.2 に下げても F は −74% のままでした。
だから「試して、悪ければ戻す」という手順にしてあります。

品質チェックが**両方向を見ている**ので、この手順が成り立ちます。VAD off の崩壊は「同一行の連続」で、
VAD on の崩壊は「音声 1 秒あたりの文字数」で捕まります。片方の指標だけでは片方向しか見えません。

無音の割合と音量は `ffmpeg` で測れます（判断材料にはなりますが、上記のとおり予測はできません）。
...
| --- | --- | --- | --- |
| `vad` | `false` | `false` | **両刃**。崩壊を直すこともあれば作ることもある（前述の A/B）。どちらに転ぶか予測できないので既定は off にし、警告が出た part だけ手で `--vad` を試す |

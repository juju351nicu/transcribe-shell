...
| 指標 | 既定 | 設定キー |
| --- | --- | --- |
| 同一行が連続した回数 | 3 行以上で警告 | `transcribe.ffm.repetition-warn-lines` |
...
既定値の根拠は実測です。崩壊した part が「同一行 55 行連続 / 1.01 文字per秒」、正常な part が
「最長 2 行 / 3.83〜5.63 文字per秒」でした（`docs/ENGINE_BENCHMARK.md`）。相槌が 2 行続くのは
普通に起きるので、3 行から警告します。0 以下を指定するとその指標を無効にできます。

**自動で `--vad` に切り替えることはしません。** VAD は発話が途切れない録音では文字数を 3 割落とすので、
崩壊の実例がまだ少ないうちに挙動を変えると、正常な録音を壊す方に転びます。

...
public class FfmProperties {
	 ...
	 * 議事録用途では網羅性を優先し既定 false。
	 *
	 * <p><b>ただし損得は録音の性質で逆転する。</b>2026-09-08 に、非発話区間が 23% を占める 10 分の
	 * part で、VAD off だと幻聴の 1 行が 55 行連続して<b>会話が 1 文も残らなかった</b>のに対し、
	 * VAD on では繰り返しが消えて 2,296 文字が取れた（初期プロンプトを外してもループしたので、
	 * プロンプトは原因ではない）。「上の 3 割減」は発話が途切れない録音での値であって、
	 * 沈黙の多い録音には当てはまらない。
	 *
	 * <p>そういう録音では {@code --vad} を手で付ける。1 録音の結果で既定を変えると、
	 * 発話が詰まった録音で 3 割落とすことになるため、既定はまだ false のまま。
	 * 数値と切り分けの経緯は {@code docs/ENGINE_BENCHMARK.md} を参照。
	 */
	 ...
	 * 2026-09-08 の実測では崩壊した part で 55 行連続、正常な part は最長 2 行だった
	 * （「はい。」のような相槌が 2 行続くのは普通に起きる）。3 に置くと正常な part を誤検知しない。
	 */
	private int repetitionWarnLines = 3;

	/**
	 * 音声 1 秒あたりの文字数がこれを下回ったら警告する（0 以下で無効）。
	 *
	 * <p>連続同一行にならない形で崩壊した場合も拾うための 2 つ目の指標。2026-09-08〜09 の実測では
	 * 崩壊した part が 1.01、正常な part が 3.83〜5.63 だった。2.5 なら両側に余裕がある。
	 * 沈黙が非常に長い録音では正常でも下回りうるが、その場合も {@code --vad} を検討する合図になる。
	 */

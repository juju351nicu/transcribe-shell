...
public class FfmProperties {
	...
	/**
	 * この文字数以下の行の連続は {@link #repetitionWarnLines} で数えない（0 以下ですべて数える）。
	 *
	 * <p>「はい。」のような相槌は自然に連続する。2026-09-10 に実際に 5 行連続して誤検知し、
	 * 3 分の再実行を無駄にした。一方、崩壊時に繰り返される行は実測 4 例すべてが 15 文字以上で、
	 * 幻聴（「私は、私のビデオを紹介します。」15 文字）も実発話の復唱（18 文字）も文の形をしている。
	 *
	 * <p>回数を 5 → 6 に上げるより長さで切る方が筋が良い。長い文が 5 行続くのは崩壊であって、
	 * 相槌が 6 行続くのは自然だからである。8 文字は実測の 3 文字と 15 文字の間に取った。
	 */
	private int repetitionMinLineLength = 8;
